...
const char* ZQ_ImageMattingGUI::winName = "Image Matting @ Zuo Qing";
const char* ZQ_ImageMattingGUI::render_winName = "render";
ZQ_DImage<double> ZQ_ImageMattingGUI::ori_image;
bool ZQ_ImageMattingGUI::has_ori_trimap = false;
ZQ_DImage<double> ZQ_ImageMattingGUI::ori_trimap;
ZQ_DImage<double> ZQ_ImageMattingGUI::trimap;
ZQ_DImage<float> ZQ_ImageMattingGUI::tex;
ZQ_DImage<float> ZQ_ImageMattingGUI::tex_alpha;
ZQ_DImage<double> ZQ_ImageMattingGUI::fore;
ZQ_DImage<double> ZQ_ImageMattingGUI::back;
ZQ_DImage<double> ZQ_ImageMattingGUI::alpha;
ZQ_ImageMattingOptions ZQ_ImageMattingGUI::opt;
IplImage* ZQ_ImageMattingGUI::background_img = 0;
IplImage* ZQ_ImageMattingGUI::zoom_background_img = 0;
IplImage* ZQ_ImageMattingGUI::draw_img = 0;
int ZQ_ImageMattingGUI::eraser_half_size = 5;
int ZQ_ImageMattingGUI::eraser_max_half_size = 50;
int ZQ_ImageMattingGUI::eraser_min_half_size = 1;
int ZQ_ImageMattingGUI::eraser_pos_x = 0;
int ZQ_ImageMattingGUI::eraser_pos_y = 0;
bool ZQ_ImageMattingGUI::eraser_has_last_pos = false;
int ZQ_ImageMattingGUI::eraser_last_pos_x = 0;
int ZQ_ImageMattingGUI::eraser_last_pos_y = 0;
bool ZQ_ImageMattingGUI::erase_mode = false;
ZQ_ImageMattingGUI::EraserType ZQ_ImageMattingGUI::eraser_type = ZQ_ImageMattingGUI::ERASER_TYPE_UNKNOWN;
bool ZQ_ImageMattingGUI::has_marker = false;
double ZQ_ImageMattingGUI::marker[8] = { 0 };
ZQ_VirtualAdRender::MarkerMode ZQ_ImageMattingGUI::marker_mode = ZQ_VirtualAdRender::MODE_ABCD;
bool ZQ_ImageMattingGUI::draw_marker = false;
bool ZQ_ImageMattingGUI::updated_flag = false;
int ZQ_ImageMattingGUI::zoom_scale = 5;
bool ZQ_ImageMattingGUI::zoom_mode = false;
int ZQ_ImageMattingGUI::zoom_center_x = 0;
int ZQ_ImageMattingGUI::zoom_center_y = 0;
int ZQ_ImageMattingGUI::cur_mouse_pos_x = 0;
int ZQ_ImageMattingGUI::cur_mouse_pos_y = 0;

bool ZQ_ImageMattingGUI::Load(const ZQ_ImageMattingOptions& option)
{
	if (!option.has_in_image)
	{
		printf("in_image_file is not specified!\n");
		return false;
	}
	...
	has_ori_trimap = option.has_in_trimap;

	if (has_ori_trimap && !ZQ_ImageIO::loadImage(ori_trimap, option.in_trimap_file,0))
	{
		printf("failed to load %s\n", option.in_trimap_file);
		return false;
	}

	has_marker = option.has_in_marker;

	if (has_marker && !ZQ_VirtualAdKeyIO::LoadMarker(option.in_marker_file, marker))
	{
	...
	case ZQ_ImageMattingOptions::RENDER_ONLY_TEX: case ZQ_ImageMattingOptions::RENDER_FBA_TEX:
		...
		if (!ZQ_VirtualAdRender::GetMarkerMode(option.marker_mode, marker_mode))
		{
	...
	{
		ori_trimap.allocate(width, height);
	}
...
void ZQ_ImageMattingGUI::_drawTrimap(IplImage* img)
{
...
void ZQ_ImageMattingGUI::_drawErazer(IplImage* img)
{
	if (!zoom_mode)
	{
...
void ZQ_ImageMattingGUI::_drawMarker(IplImage* img)
{
	if (!zoom_mode)
	{
		if (has_marker)
...
void ZQ_ImageMattingGUI::_mouseHandler(int event, int x, int y, int flags, void* param)
{
	cur_mouse_pos_x = x;
	cur_mouse_pos_y = y;
...
void ZQ_ImageMattingGUI::_erase()
{
	int width = trimap.width();
	int height = trimap.height();
	double*& trimap_data = trimap.data();
		...
		&& eraser_pos_x >= 0 && eraser_pos_x < width && eraser_pos_y >= 0 && eraser_pos_y < height)
	{
		...
		ZQ_ScanLinePolygonFill::FillOneStrokeWithClip(ZQ_Vec2D(eraser_last_pos_x, eraser_last_pos_y), ZQ_Vec2D(eraser_pos_x, eraser_pos_y), eraser_half_size, width, height, pixels);
...
void ZQ_ImageMattingGUI::_saveAsBack()
{
	int width = ori_image.width();
	int height = ori_image.height();
	int nChannels = ori_image.nchannels();
...
void ZQ_ImageMattingGUI::_renderAndShow()
{
		...
		|| opt.render_mode == ZQ_ImageMattingOptions::RENDER_FBA_TEX_WITH_ALPHA)
	{
		...
		{
		case ZQ_ImageMattingOptions::RENDER_ONLY_TEX: case ZQ_ImageMattingOptions::RENDER_FBA_TEX:
			if (!ZQ_VirtualAdRender::Render_fba(out_im, fore_f, back_f, alpha_f, tex, marker, marker_mode))
			{
			...
			break;
		case ZQ_ImageMattingOptions::RENDER_TEX_WITH_ALPHA:case ZQ_ImageMattingOptions::RENDER_FBA_TEX_WITH_ALPHA:
			if (!ZQ_VirtualAdRender::Render_fba_with_alpha(out_im, fore_f, back_f, alpha_f, tex, tex_alpha, marker, marker_mode))
			{
...
void ZQ_ImageMattingGUI::_auto_matting()
{
	if (!has_ori_trimap)
	{
		ZQ_DImage<double> tmp_trimap;
		if (ZQ_LazySnappingGUI::Run(ori_image, tmp_trimap))
		{
			ori_trimap = tmp_trimap;
			printf("specify trimap by lazy snapping done!\n");
		}
		else
		{
	...
	else
	{
		for (int i = 0; i < width*height; i++)
		{
			if (!fore_map_data[i] && !back_map_data[i])
			{
				trimap_data[i] = 0.5;
