...
const char* ZQ_PlayTrackingGUI::winName = "Play Tracking @ Zuo Qing";
const char* ZQ_PlayTrackingGUI::match_winName = "match";
IplImage* ZQ_PlayTrackingGUI::background_img = 0;
IplImage* ZQ_PlayTrackingGUI::draw_img = 0;
int ZQ_PlayTrackingGUI::cur_frame_id = 0;
bool ZQ_PlayTrackingGUI::updated_flag = false;
bool ZQ_PlayTrackingGUI::is_paused = true;
bool ZQ_PlayTrackingGUI::draw_marker = true;
bool ZQ_PlayTrackingGUI::has_marker = false;
bool ZQ_PlayTrackingGUI::has_polygon = false;
std::vector<ZQ_Vec2D> ZQ_PlayTrackingGUI::poly;
double ZQ_PlayTrackingGUI::marker[8] = { 0 };
bool ZQ_PlayTrackingGUI::draw_info = true;
ZQ_PlayTrackingConfig ZQ_PlayTrackingGUI::config;
ZQ_DImage<bool> ZQ_PlayTrackingGUI::poly_keyframe_flag;
ZQ_DImage<bool> ZQ_PlayTrackingGUI::marker_keyframe_flag;
char ZQ_PlayTrackingGUI::config_filename[200];

bool ZQ_PlayTrackingGUI::LoadConfig(const char* file)
{
...
void ZQ_PlayTrackingGUI::_mouseHandler(int event, int x, int y, int flags, void* param)
{

}

bool ZQ_PlayTrackingGUI::_load_first_frame()
{
	if (config.frame_num <= 0)
		return false;
	int tmp_frame_id = 0;
	...
	if (tmp_img != 0)
	{
		...
		has_marker = ZQ_VirtualAdKeyIO::LoadMarker(filename, marker);
		sprintf(filename, "%s\\%s\\%d.txt", config.work_fold, config.poly_fold, config.base_id + cur_frame_id);
		has_polygon = ZQ_VirtualAdKeyIO::LoadPolygonConfig(filename, poly);
...
void ZQ_PlayTrackingGUI::_load_next_keyframe()
{
	if (config.frame_num <= 0)
		return;
...
void ZQ_PlayTrackingGUI::_load_previous_frame()
{
...
void ZQ_PlayTrackingGUI::_load_next_frame()
{
	if (config.frame_num > 1)
	{
...
void ZQ_PlayTrackingGUI::_draw_marker(IplImage* draw_img)
{
...
void ZQ_PlayTrackingGUI::_draw_polygon(IplImage* draw_img)
{
...
void ZQ_PlayTrackingGUI::_draw_info(IplImage* draw_img)
{
	CvFont font;
...
void ZQ_PlayTrackingGUI::_select_polygon()
{
	std::vector<ZQ_Vec2D> tmp_poly;
	ZQ_SelectPolygonGUI::Select(background_img, tmp_poly, 50, 1, "select polygon");
	if (tmp_poly.size() >= 3)
	{
		char filename[200];
		sprintf(filename, "%s\\%s\\%d.txt", config.work_fold, config.poly_fold, config.base_id + cur_frame_id);
		if (ZQ_VirtualAdKeyIO::SavePolygonConfig(filename, tmp_poly))
		{
			poly = tmp_poly;
...
void ZQ_PlayTrackingGUI::_select_marker()
{
	std::vector<ZQ_Vec2D> tmp_poly;
	ZQ_SelectPolygonGUI::Select(background_img, tmp_poly, 4, 1, "select marker");
	if (tmp_poly.size() == 4)
	{
		double tmp_marker[8] = {
		...
		char filename[200];
		sprintf(filename, "%s\\%s\\%d.txt", config.work_fold, config.marker_fold, config.base_id + cur_frame_id);
		if (ZQ_VirtualAdKeyIO::SaveMarker(filename, tmp_marker))
		{
...
void ZQ_PlayTrackingGUI::_save_config()
{
...
void ZQ_PlayTrackingGUI::_propagate_marker(int key_id)
{
...
void ZQ_PlayTrackingGUI::_remove_marker_keyframe(int key_id)
{
	if (key_id < 0 || key_id >= config.frame_num)
		return;
	int previous_key_id = 0;
...
void ZQ_PlayTrackingGUI::_propagate_marker_after_polygon(int poly_key_id)
{
...
void ZQ_PlayTrackingGUI::_propagate_marker(int start_id, int end_id)
{
	...
	for (int i = 0; i < N - 1; i++)
	{
		...
		if (!ZQ_VirtualAdKeyIO::Load_H_invH(filename, H + i * 9, invH + i * 9))
		{
	...
	if (keyframe_mask_data[0])
	{
		...
		if (!ZQ_VirtualAdKeyIO::LoadMarker(filename, marker_data))
		{
			printf("failed to load %s\n", filename);
			return;
		}
	...
	if (keyframe_mask_data[N - 1])
	{
		...
		if (!ZQ_VirtualAdKeyIO::LoadMarker(filename, marker_data + 8 * (N - 1)))
		{
			printf("failed to load %s\n", filename);
			return;
		}
	...
	for (int i = 0; i < N; i++)
	{
		...
		if (!keyframe_mask_data[i])
		{
			if (!ZQ_VirtualAdKeyIO::SaveMarker(filename, marker_data + i * 8))
			{
...
void ZQ_PlayTrackingGUI::_propagate_polygon(int key_id)
{
	if (key_id < 0 || key_id >= config.frame_num)
		return;
...
void ZQ_PlayTrackingGUI::_propagate_polygon(int start_id, int end_id)
{
	if (start_id < 0 || end_id >= config.frame_num || start_id >= end_id)
		return;
	...
	for (int fr = 0; fr < N; fr++)
	{
		...
		if (fr == 0)
		{
			...
			if (!ZQ_VirtualAdKeyIO::LoadSiftSurfKey(filename, num, dim, coords, vals))
			{
				printf("failed to load %s\n", filename);
				return;
			}
			sprintf(filename, "%s\\%s\\%d.txt", config.work_fold, config.poly_fold, fr_id);
			if (!ZQ_VirtualAdKeyIO::LoadPolygonConfig(filename, polygon))
			{
				delete[]coords;
				delete[]vals;
				printf("failed to load %s\n", filename);
		...
		else
		{
			...
			if (!ZQ_VirtualAdKeyIO::Save_H_invH(filename, H, invH))
			{
				printf("failed to save %s\n", filename);
				return ;
			}
			...
			sprintf(filename, "%s\\%s\\%d.txt", config.work_fold, config.poly_fold, fr_id);
			if (!ZQ_VirtualAdKeyIO::SavePolygonConfig(filename, polygon))
			{
				printf("failed to save %s\n", filename);
				return ;
			}
